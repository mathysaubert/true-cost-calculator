...
// fiabilité pointent la page Réglages (ou l'écran classique pour les coûts par variante, jusqu'à S3).
const num = (v) => { const n = typeof v === "number" ? v : parseFloat(v); return Number.isFinite(n) ? n : null; };

// Règle de fiabilité → page où la corriger. Coûts par variante et coût rendu : écran classique (Suivi des coûts).
export const RULE_PAGES = {
  cost_coverage: { path: "/app", section: "legacy" },
  landed_cost: { path: "/app", section: "legacy" },
...
export function activationChecklist({ lastSync = null, confidence = null, briefing = null, ordersInPeriod = 0 } = {}) {
  ...
  const items = [
    ...
    { id: "costs", done: costShare != null && costShare >= COSTS_DONE_SHARE, path: "/app", detail: costShare == null ? null : Math.round(costShare * 100) },
