...
export function unitEconomics({ inputs = {}, shopCountryCode = null, now = new Date() } = {}) {
  ...
  const landed = landedCostUnit({
    costRow: { prix_achat: v.prix_achat, port_entrant: v.port_entrant ?? 0, qty_par_lot: v.qty_par_lot ?? 1, vat_regime: vatRegime, shipping_model: inputs.shipping_model === "stock" ? "stock" : "dropshipping", categorie: inputs.categorie ?? "Autre", duty_rate_pct: v.duty_rate_pct ?? 0 },
    shopCountryCode, now,
  });
  const saleVat = vatRegime === "franchise" ? 0 : vatRateFor({ countryCode: shopCountryCode, categorie: inputs.categorie }) ?? 0;
  const priceHt = v.price_ttc / (1 + saleVat);
  const fees = v.price_ttc * (v.payment_pct ?? 0) / 100 + (v.payment_fixed ?? 0);
  ...
  return {
    ok: true,
    price_ttc: round2(v.price_ttc), price_ht: round2(priceHt), sale_vat_rate: saleVat,
