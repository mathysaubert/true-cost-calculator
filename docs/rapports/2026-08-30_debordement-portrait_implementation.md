# Débordement portrait-seulement (iOS) — rapport d'implémentation option B

Date : 2026-08-30. Fait suite à la Phase 0 de réouverture
(`docs/rapports/2026-08-30_debordement-portrait_phase0.md`). Option A (mesure) écartée par
Mathys sur preuve desktop : rendu PROPRE en responsive DevTools à 768 ET 390 px → le
débordement n'existe que dans le WebView de l'app Shopify iOS native, coupable dans
l'encapsulation App Bridge/iOS, hors dépôt, non inspectable. Aucun commit : GO séparé
attendu (R1). **Livraison conditionnée à la capture iPhone portrait réelle de Mathys — la
seule preuve valable, le desktop ne reproduisant pas le bug.**

---

## Problème

En portrait iPhone (~390 px), l'onglet Suivi des coûts élargissait le viewport de layout du
WebView d'environ 100 px (bande de vide à droite, tout le contenu « coupé », page qui panne,
élargissement persistant sur les autres onglets). Paysage (~850 px) et desktop (y compris
DevTools responsive 390 px) : corrects.

## Cause

Débordement horizontal INVISIBLE (~100 px) au rendu immédiat de l'onglet, non reproductible
dans un moteur desktop à largeur égale : il appartient à l'encapsulation App Bridge/WebView
iOS (hors dépôt). La Phase 0 a exclu, preuves à l'appui, tout le DOM applicatif rendu au
chargement (table d'exclusion en 11 entrées), réfuté l'hypothèse 100vw (grep), et innocenté
les tables 820/480/380 px (non rendues au chargement ; auraient débordé aussi en paysage).

## Solution (contention, aveugle mais garantie côté app)

Une règle dans la media query mobile EXISTANTE (≤ 768 px) :

```css
body { overflow-x: clip; }
```

`clip` — PAS `hidden` : pas de conteneur de défilement introduit, `position: sticky` non
affecté, pas de propagation du overflow au viewport. Le body n'expose plus de zone défilable
horizontale → le WebView n'a plus de contenu plus large que l'écran à épouser → le viewport
ne s'élargit plus, quel que soit le coupable, y compris hors de notre DOM (body est
l'ancêtre de tout le contenu de l'iframe). Rien n'est retiré : `tcc-scroll-x` (tables) et
`-webkit-text-size-adjust` (7587e43) restent en place.

## Vérification « aucun défilement légitime cassé » (exigée par le GO)

Grep `sticky|overflowX|overflow-x` sur `app/**` — résultat exhaustif :

- `position: sticky` : **zéro occurrence CSS** dans l'app (les seuls hits « sticky » sont un
  nom de variable JS dans `app/lib/customsClassification.js:67-68`, logique pure sans CSS).
- Défilements horizontaux VOULUS : exactement les trois wrappers `.tcc-scroll-x`
  (`overflowX: "auto"` local — `costsUi.jsx:161`, `app._index.jsx:1780`, `:1971` : panneau
  d'édition et monitor). Un `overflow-x: clip` sur le body ne supprime que la zone défilable
  du body LUI-MÊME ; les conteneurs de défilement descendants gardent leur défilement
  interne → les tables défilent toujours au doigt dans leur cadre.
- Seule autre règle : `.tcc-history-container { overflow-x: hidden; }` (mobile, onglet
  Historique, `:2889`) — déjà une contention, non affectée.

## Preuves

### Diff complet (1 fichier, 3 insertions, 0 suppression)

```diff
diff --git a/app/routes/app._index.jsx b/app/routes/app._index.jsx
@@ (bloc <style>, media query mobile existante)
...
Diff attendu 1-2 lignes ; réel : 1 ligne de code + 1 commentaire + 1 ligne vide = 3
insertions. Dans la marge I3.

### Gate complète

`npm run lint` 0 erreur ; `npm test` **22/22 lots verts** ;
`node scripts/render_check.mjs` **« ✅ Tous les rendus réels OK »** (35 scénarios — desktop
intact : la règle vit sous ≤ 768 px, inactive au-delà) ; `npm run build` succès. Aucun test
modifié. Zéro nouvelle chaîne, `engine.js` intouché.

### E1 — procédure de capture (par Mathys, POST-déploiement ; seule preuve valable)

1. GO commit + push (message séparé) → build + déploiement Vercel.
2. Sur iPhone : FERMER complètement l'app admin Shopify puis la rouvrir (l'élargissement
   résiduel de la session en cours peut survivre au déploiement).
3. En PORTRAIT : ouvrir l'app → onglet Suivi des coûts → vérifier dès l'affichage (avant
   même le chargement de la liste) : aucun vide à droite, aucun pan horizontal de la page,
   en-tête et blocs à pleine largeur d'écran.
4. Ouvrir un panneau produit : la table des coûts doit encore défiler AU DOIGT dans son
   cadre (le clip du body ne doit pas l'avoir gelée) — point de vérification spécifique.
5. Repasser Calculateur/Historique en portrait : échelle normale (plus de contamination).
6. Basculer paysage puis retour portrait : stable.
7. Envoyer la capture portrait (onglet affiché + un panneau ouvert). Livraison à ta
   confirmation ; si le vide persiste, la piste résiduelle est un élargissement AMONT du
   WebView lui-même (hors document) — à remonter alors côté App Bridge/Shopify.

---

## Annexe L2 — brouillon d'entrée pour docs/CHANTIERS_DETAIL.md (NON committé, à recopier)

### Débordement portrait iOS — contention body (option B)

- **Problème** : viewport de layout élargi (~+100 px) en portrait iPhone sur l'onglet Suivi
  des coûts, débordement invisible (vide), contamination des autres onglets ; absent en
  paysage, sur desktop et en DevTools responsive 390 px.
- **Cause** : hors dépôt (encapsulation App Bridge/WebView iOS) — DOM applicatif
  intégralement exclu par table de preuves (Phase 0 de réouverture) ; non nommable sans
  outillage iOS, recherche arrêtée sur décision (non rentable).
- **Solution** : `body { overflow-x: clip; }` dans la media query mobile existante — clip
  (pas hidden) : aucune zone défilable horizontale exposée au WebView, sticky (inexistant
  dans l'app) et défilements locaux `.tcc-scroll-x` préservés. Les fixes 7587e43
  (tcc-scroll-x, text-size-adjust) restent en place.
- **Preuves** : gate 22 lots + render_check 35 scénarios (desktop intact) ; capture iPhone
  portrait réelle post-déploiement (seule preuve du bug WebView), avec vérification que les
  tables défilent encore au doigt.
