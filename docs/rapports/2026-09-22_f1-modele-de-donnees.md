# Chantier F1 — modèle de données de la refonte : rapport d'écriture

Date : 2026-09-22. Fait suite aux décisions consignées dans
`docs/rapports/2026-09-22_decisions.md` (sections C.2 à C.6). État : **fichiers écrits, rien
appliqué, rien committé**. Deux GO restent nécessaires : appliquer (base de test puis prod),
puis committer/pusher.

---

## Problème

La refonte exige un schéma qui porte le graphe économique (CM1 → CM2 → CM3 → résultat), les
faits temps réel (remboursements, retours, expéditions, frais réels, stock), les sources
externes (pub, SEO, sessions), les saisies marchand et l'orchestration (jobs, webhooks,
alertes, cache IA) — sans casser l'app en production le jour de l'application, sans réécrire
un seul snapshot de marge, et avec une purge RGPD qui ne dépende plus d'une liste dupliquée
dans deux webhooks.

## Cause (pourquoi le schéma actuel ne suffit pas)

Douze tables conçues pour un calculateur et un monitor 30 jours : pas de table de commandes
(seules les lignes de marge existent), pas de remboursements/retours/expéditions/frais réels,
un état de sync à une ligne par boutique, un état d'alerting mono-type, des réglages métier
mélangés à la facturation dans `shop_plans`, et une liste de purge RGPD copiée-collée dans
`webhooks.app.uninstalled.jsx` et `webhooks.compliance.jsx`.

## Solution

### 23 migrations `supabase/migrations/20260922_f1_*.sql` (26 tables, 2 fonctions)

| # | Fichier | Crée / modifie |
|---|---|---|
| 01 | `shop_settings` | réglages métier scindés de `shop_plans` + copie idempotente des réglages existants (retour 30 j, fuseau figé, règles de frais par passerelle « à confirmer », 24 mois) |
| 02 | `orders` | faits par commande : montants HT/TTC depuis `taxLines`, remises, codes, port client (dans le CA brut, d1), attribution UTM/`CustomerVisit`, `customer_id` gid brut (b), `country_code` seul (c), `purchasing_company_id` (d9), `excluded_reason` avec CHECK |
| 03 | `order_margins` étendue | 8 colonnes NULLABLES sans DEFAULT (HT, taxes, carte cadeau, composantes CM1/CM2, version, `day_local`) ; NI rename NI drop |
| 04–07 | `refunds`, `returns`, `fulfillments`, `order_fees` | remboursements réels, retours avec motif (enum Shopify), expéditions avec `delivered_at`/`promised_at`, frais réels ou règle manuelle (`confirmed`, d16) |
| 08–12 | `inventory_daily`, `sessions_daily`, `ad_spend` + `ad_entities`, `seo_daily` + `seo_index_status`, `fx_rates` | stock quotidien, sessions agrégées (aucune donnée nominative), pub par jour × niveau × pays, SEO, taux de change (référentiel partagé, hors purge) |
| 13 | `variant_costs` étendue | délai fournisseur, tampon, taux de droits hors UE, coût rendu saisi (d7) ; `product_id` reste nullable (a) |
| 14–15 | `partners`, `promo_code_rules`, `manual_commissions`, `fixed_costs` | commissions par code (base réglable, défaut HT après remise, d2), partenaires codes OU manuel, coûts fixes |
| 16 | `integration_connections` | jetons chiffrés (colonnes TEXT base64, d20), statut, révocation |
| 17 | `sync_jobs` | un job = un message (d19) ; copie unique d'`order_sync_state`, conservée jusqu'à F2 |
| 18 | `webhook_events` | idempotence des webhooks (`X-Shopify-Webhook-Id`) |
| 19 | `daily_rollups`, `product_daily_rollups` | cache reconstructible ; MÊME jeu de colonnes de métriques → future `variant_daily_rollups` sans toucher au reste (f) |
| 20 | `customers_agg` | cohortes/LTV, gid brut, niveau 1 |
| 21 | `alert_rules` (table, d) + `alert_state` | généralise `product_profitability_state` (copie unique, conservée jusqu'à I1) |
| 22 | `ai_explanations` | cache d'empreinte (d12) |
| 23 | fonctions `purge_shop(p_shop)`, `redact_customer(p_shop, p_customer_id)` | purge TOTALE des 37 tables porteuses de `shop_domain` (g), enfants avant parents ; effacement client = ligne `customers_agg` supprimée + commandes anonymisées, jamais supprimées |

Reportés volontairement (décisions C.5) : le renommage `order_margins → order_lines` (F2,
avec le code) et la suppression de `calculations`/`calculation_annotations`/`margin_alerts`
(chantier qui retire le code, avec un `CREATE OR REPLACE` de `purge_shop`).

### Code applicatif (aucune UI)

- `app/lib/schema.js` (pur) : listes `LEGACY_TABLES` (12), `F1_TABLES` (26), `PURGE_TABLES`
  (37, enfants d'abord), `ORDER_MARGINS_KEY`, `ORDER_MARGINS_SNAPSHOT_COLUMNS`,
  `ORDER_EXCLUSION_REASONS` — le garde-fou contre une table oubliée par la purge.
- `app/lib/crypto.server.js` : AES-256-GCM, clé `TOKEN_ENCRYPTION_KEY` (32 octets base64) lue à
  l'appel ; clé absente ou invalide → erreur explicite, jamais de stockage en clair.
- `webhooks.app.uninstalled.jsx` et `webhooks.compliance.jsx` : appellent la RPC `purge_shop` ;
  si elle est absente (migration pas encore appliquée) ou échoue, repli table par table piloté
  par `PURGE_TABLES` — plus AUCUN nom de table en dur, comportement d'avant conservé à
  l'identique tant que la migration n'est pas appliquée. `customers/redact` appelle
  `redact_customer` avec le gid brut. Sécurité de déploiement : l'ordre « migration puis
  déploiement » n'est PLUS critique grâce au repli, mais reste l'ordre recommandé.
- `tests/lot23_schema.mjs` branché à la gate (`package.json`).

## Preuves

### Lot 23 — 95 assertions, toutes vertes (sortie intégrale en annexe A)

1. RLS + `deny_public_access` (précédée de `DROP POLICY IF EXISTS`) sur les 26 tables, dans le
   même fichier que leur création.
2. Ré-exécutabilité des 23 fichiers (`IF NOT EXISTS` tables/index/colonnes, `OR REPLACE`
   fonctions, autant de `CREATE POLICY` que de `DROP POLICY IF EXISTS`).
3. Contrat de liste : `schema.js` = exactement l'ensemble des tables créées par TOUTES les
   migrations (les 12 historiques ont été retrouvées dans les anciens fichiers ; 12 + 26,
   aucun doublon).
4. `purge_shop` vide chaque table de `PURGE_TABLES` et rien d'autre, une seule fois, enfants
   avant parents ; `fx_rates` hors purge ; les deux webhooks appellent la RPC, importent la
   liste partagée et ne contiennent aucun `.from("…")` en dur ; `customers/redact` appelle
   `redact_customer` avec `gid://shopify/Customer/`.
5. `order_margins` : clé `(shop_domain, order_id, line_item_id)` présente dans la migration
   d'origine ; 8 colonnes ajoutées, toutes sans `DEFAULT` ni `NOT NULL` ; aucun `RENAME`/`DROP` ;
   aucune migration F1 ne fait d'`UPDATE` sur une colonne de snapshot ; le CHECK
   `excluded_reason` = `ORDER_EXCLUSION_REASONS`.
6. Chiffrement : aller-retour UTF-8, IV aléatoire, jeton absent du texte chiffré, clé
   absente/mauvaise longueur/mauvaise clé → erreur.
7. `redact_customer` : supprime `customers_agg`, anonymise `orders`, ne supprime aucune
   commande.

### Gate complète

`npm run lint` 0 erreur (345 warnings, politique du dépôt) ; `npm test` **23/23 lots** ;
`node scripts/render_check.mjs` « Tous les rendus réels OK » (35 scénarios — aucune surface
rendue touchée, `engine.js` intouché) ; `npm run build` succès.

### Périmètre du diff (`git status`)

Modifiés : `app/routes/webhooks.app.uninstalled.jsx`, `app/routes/webhooks.compliance.jsx`,
`package.json`. Nouveaux : `app/lib/schema.js`, `app/lib/crypto.server.js`,
`tests/lot23_schema.mjs`, 23 fichiers `supabase/migrations/20260922_f1_*.sql`. Diff des
fichiers suivis en annexe B. Aucune migration appliquée (aucune commande d'écriture vers une
base n'a été lancée).

---

## Procédure d'application (second GO, à exécuter par toi — je ne lance rien qui écrive)

Pré-requis : `.env.test` rempli (C.6 des décisions), table `Session` créée sur la base de
test (`npx prisma migrate deploy` avec `DATABASE_URL`/`DIRECT_URL` de test chargés).

1. **Base de test d'abord.** Depuis le dépôt, en PowerShell :
   `supabase link --project-ref <ref-du-projet-de-test>` (mot de passe demandé, jamais dans
   le chat), puis `supabase db push` — le CLI applique dans l'ordre alphabétique : les 20
   fichiers historiques sont déjà marqués appliqués si le projet a été initialisé par le même
   CLI ; sinon, appliquer les 23 fichiers F1 un par un dans le SQL Editor, dans l'ordre 01 → 23
   (chacun est ré-exécutable).
2. **Preuve de lecture seule** (je la produis après ton signal) : `information_schema.tables`
   + `pg_policies` → 26 nouvelles tables avec RLS et politique, 2 fonctions ; puis lecture des
   lignes copiées (`shop_settings`, `sync_jobs`, `alert_state`) comparées aux tables sources.
3. **Test E2E documenté** sur la base de test : `SELECT public.purge_shop('exemple.myshopify.com')`
   sur une boutique fictive insérée exprès, puis vérification que les 37 tables sont vides pour
   ce domaine ; `redact_customer` de même. Lecture seule de ma part, exécution par toi.
4. **Prod ensuite**, même commande après `supabase link` sur le projet de prod — uniquement
   après validation du point 2 sur la base de test. Ordre recommandé : migrations en prod PUIS
   déploiement du code (le repli des webhooks couvre l'ordre inverse).
5. Puis troisième GO : commit unique + push.

## Preuve empirique — application sur la base de TEST (2026-09-22, après second GO)

Outil : `prisma migrate deploy` (table `Session`) puis `prisma db execute --file <migration>
--url $env:DIRECT_URL` (Prisma 6.19.3, déjà installé — ni `pg` ni le CLI Supabase ne sont sur
le poste), piloté par un script de scratchpad qui charge `.env.test` en variables de processus
et masque toute URL. Cible confirmée par l'hôte affiché par Prisma : `aws-0-eu-central-1`
(projet de test ; la prod est en `eu-west-1`). Correction préalable : F1-01 ajoute
`shop_plans.shipping_model` de façon idempotente (colonne créée à la main en prod, absente
de toute migration — un rejeu sur base vide échouait sinon).

Application (sortie brute, 43 fichiers = 20 historiques + 23 F1, ordre alphabétique) :

```text
== 1. prisma migrate deploy (table Session) ==
Applying migration `20240530213853_create_session_table`
All migrations have been successfully applied.

== 2. migrations Supabase (ordre alphabetique) ==
[ 1/43] OK     001_create_calculations.sql
[ 2/43] OK     002_usage_tracking.sql
[ 3/43] OK     003_update_calculations.sql
[ 4/43] OK     004_margin_alerts.sql
[ 5/43] OK     20260524_annotations.sql
[ 6/43] OK     20260529_rate_limits.sql
[ 7/43] OK     20260530_initial_schema.sql
[ 8/43] OK     20260609_shop_plans.sql
[ 9/43] OK     20260611_rls_policies.sql
[10/43] OK     20260612_shop_plans_vat_regime.sql
[11/43] OK     20260622_order_margins.sql
[12/43] OK     20260622_variant_costs.sql
[13/43] OK     20260627_order_margins_breakdown.sql
[14/43] OK     20260701_product_profitability_state.sql
[15/43] OK     20260702_shop_plans_profitability_threshold.sql
[16/43] OK     20260703_subscription_dunning_state.sql
[17/43] OK     20260704_session_health.sql
[18/43] OK     20260712_shop_plans_current_cpa.sql
[19/43] OK     20260716_usage_orders_count.sql
[20/43] OK     20260722_variant_costs_customs_confirmed.sql
[21/43] OK     20260922_f1_01_shop_settings.sql
[22/43] OK     20260922_f1_02_orders.sql
[23/43] OK     20260922_f1_03_order_margins_extend.sql
[24/43] OK     20260922_f1_04_refunds.sql
[25/43] OK     20260922_f1_05_returns.sql
[26/43] OK     20260922_f1_06_fulfillments.sql
[27/43] OK     20260922_f1_07_order_fees.sql
[28/43] OK     20260922_f1_08_inventory_daily.sql
[29/43] OK     20260922_f1_09_sessions_daily.sql
[30/43] OK     20260922_f1_10_ad_spend.sql
[31/43] OK     20260922_f1_11_seo_daily.sql
[32/43] OK     20260922_f1_12_fx_rates.sql
[33/43] OK     20260922_f1_13_variant_costs_extend.sql
[34/43] OK     20260922_f1_14_partners_promo_codes.sql
[35/43] OK     20260922_f1_15_fixed_costs.sql
[36/43] OK     20260922_f1_16_integration_connections.sql
[37/43] OK     20260922_f1_17_sync_jobs.sql
[38/43] OK     20260922_f1_18_webhook_events.sql
[39/43] OK     20260922_f1_19_daily_rollups.sql
[40/43] OK     20260922_f1_20_customers_agg.sql
[41/43] OK     20260922_f1_21_alerts.sql
[42/43] OK     20260922_f1_22_ai_explanations.sql
[43/43] OK     20260922_f1_23_rgpd_functions.sql
Termine : 43 fichiers appliques.
```

Vérification LECTURE SEULE (`information_schema.tables`, `pg_class.relrowsecurity`,
`pg_policies`, `pg_proc`, `information_schema.columns`), comparée au contrat `schema.js` :
40 tables présentes (38 du contrat + `Session` + `_prisma_migrations`), aucune hors contrat ;
RLS activée + politique `deny_public_access` sur chacune des 26 tables F1 ; fonctions
`purge_shop`, `redact_customer`, `increment_usage_orders` présentes ; les 8 colonnes ajoutées
à `order_margins` nullables et sans DEFAULT ; comptages
`{"shop_plans":0,"shop_settings":0,"order_sync_state":0,"sync_jobs":0,"product_profitability_state":0,"alert_state":0}`
(base vide — les copies sont trivialement complètes ici ; elles seront réellement exercées en
prod, qui a des lignes) → **« VÉRIFICATION : ✓ schéma conforme au contrat »**.

Deux observations sur le rejeu des migrations historiques depuis zéro (sans effet sur le
code, à connaître) : `usage` et `calculation_annotations` prennent la forme des fichiers
`002` et `20260524` (antérieurs à `initial_schema`, dont les `CREATE TABLE IF NOT EXISTS` sont
alors sans effet) — `usage` a un `id` et une contrainte UNIQUE (compatible avec
`increment_usage_orders`), `calculation_annotations.calculation_id` est TEXT sans FK. La prod,
créée à la main, peut porter l'une ou l'autre forme ; `purge_shop` gère les deux (annotations
vidées avant calculations).

### E2E RGPD sur la base de test (2026-09-22, après GO dédié) — sortie brute

Script de scratchpad (non committé) : sème une boutique fictive `e2e-rgpd.myshopify.com`
(une ligne dans CHACUNE des 37 tables de `PURGE_TABLES`, deux clients A et B dans `orders` et
`customers_agg`) et une boutique témoin, exécute les deux fonctions, vérifie, nettoie.

```text
── 1. Semis boutique fictive + témoin ──
  ✓ un INSERT de test pour chaque table de PURGE_TABLES
  ✓ chaque table a ≥ 1 ligne pour e2e-rgpd.myshopify.com (37 tables)

── 2. redact_customer(client A) ──
  ✓ customers_agg du client A supprimée
  ✓ customers_agg du client B intacte
  ✓ commande du client A anonymisée (id et rang NULL)
  ✓ commande du client B intacte
  ✓ aucune commande supprimée

── 3. purge_shop(boutique fictive) ──
  ✓ 0 ligne restante pour e2e-rgpd.myshopify.com dans les 37 tables
  ✓ la boutique témoin est intacte (isolation par shop_domain)

── 4. Nettoyage du témoin ──
  ✓ témoin purgé, base de test de nouveau vide pour ces boutiques

E2E RGPD : ✓ purge_shop et redact_customer conformes
```

Lecture : la purge totale (décision g) vide bien les 37 tables porteuses de `shop_domain` en un
appel, l'ordre enfants→parents passe les FK (`manual_commissions`/`promo_code_rules` →
`partners`), l'isolation par boutique tient (témoin intact), et `redact_customer` anonymise
sans supprimer. La base de test est revenue à vide.

### Retour en arrière — `supabase/rollback/20260922_f1_rollback.sql`, prouvé sur la base de test

Script HORS `supabase/migrations/` (un `db push` ne l'appliquera jamais), idempotent
(`IF EXISTS` sur chaque `DROP`) : 2 fonctions, 26 tables (`CASCADE`, enfants avant `partners`),
8 colonnes d'`order_margins`, 4 de `variant_costs`, 3 index. Aucune donnée historique touchée
(aucun `DELETE`/`TRUNCATE`/`UPDATE`) : F1 ne fait que copier, jamais déplacer. Exception
assumée et documentée : `shop_plans.shipping_model` n'est pas retirée (pré-existante en prod,
écrite par le code en production — la retirer casserait l'app). Lot 23, section 8 : 12
assertions statiques vérifient que le rollback défait exactement ce que F1 crée.

Preuve sur la base de test (sortie brute, chaque étape conditionnée à la précédente) :

```text
===== B. ROLLBACK sur la base de test =====
[ 1/1] OK     20260922_f1_rollback.sql
===== C. Verification d'ABSENCE =====
  ✓ aucune des 26 tables F1 ne subsiste
  ✓ purge_shop et redact_customer supprimées
  ✓ increment_usage_orders (historique) conservée
  ✓ les 12 tables historiques + Session sont intactes
  ✓ order_margins : colonnes F1 retirées
  ✓ order_margins : colonnes historiques conservées
  ✓ variant_costs : colonnes F1 retirées
  ✓ variant_costs : colonnes historiques conservées
  ✓ index F1 retirés
  ✓ index historiques conservés
ABSENCE F1 : ✓ retour en arrière complet
===== D. REAPPLICATION F1 (filtre) =====
[ 1/23] OK … [23/23] OK     20260922_f1_23_rgpd_functions.sql
===== E. Verification de CONFORMITE =====
VÉRIFICATION : ✓ schéma conforme au contrat
```

### Application en PRODUCTION (2026-09-22, GO conditionné au rollback prouvé)

1. **État initial, lecture seule** (même script d'absence, `.env` prod) : aucune trace de F1,
   12 tables historiques + `Session` intactes, `increment_usage_orders` présente — la prod
   était exactement dans l'état attendu.
2. **Application des 23 fichiers F1 UNIQUEMENT** (filtre `20260922_f1_*.sql`, sans
   `prisma migrate deploy`) — région `eu-west-1` confirmée par le script, aucune URL affichée :
   `[ 1/23] OK … [23/23] OK`, `Termine : 23 fichiers appliques (region eu-west-1).` Les 20
   migrations historiques n'ont pas été rejouées (20260611 pose des `CREATE POLICY` sans
   `IF EXISTS` : elle aurait échoué sur la prod où elles existent déjà).
3. **Conformité, lecture seule** : 40 tables (38 du contrat + `Session` + `_prisma_migrations`),
   aucune hors contrat ; RLS + `deny_public_access` sur les 26 tables F1 ; 3 fonctions ; 8
   colonnes nullables sans DEFAULT ; **copies réellement exercées** :
   `{"shop_plans":4,"shop_settings":4,"order_sync_state":3,"sync_jobs":3,"product_profitability_state":0,"alert_state":0}`
   → **« VÉRIFICATION : ✓ schéma conforme au contrat »**.

Incident d'outillage sans conséquence : la première vérification prod a échoué (`P1012`,
« the URL must start with postgresql:// ») parce que le script d'application avait chargé les
variables du `.env` prod avec leurs guillemets dans le shell courant et que Node privilégie
l'environnement hérité sur `--env-file` ; relancée dans un shell neuf, la vérification est
passée. Le chargeur retire désormais les guillemets. Rien n'a été écrit en prod en dehors des
23 fichiers.

Reste à faire : commit unique + push (GO). Le code en production (version 4c6e7b1) continue de
fonctionner sur le schéma étendu : aucune table ni colonne historique n'a changé, les webhooks
RGPD basculeront sur `purge_shop` au prochain déploiement (le repli couvre l'intervalle).

## Limites et points ouverts

- Les fonctions `purge_shop`/`redact_customer` et le lot 23 ne sont validés que statiquement
  jusqu'à l'application sur la base de test (étape 3) : c'est la preuve empirique attendue.
- `partners.mode` (codes OU manuel) est une règle applicative : aucun CHECK SQL inter-tables
  ne peut la porter ; elle sera testée dans le module 10.6.
- `variant_costs.product_id` reste nullable (décision a) : la première sync F2 le corrige.

---

## Annexe L2 — brouillon d'entrée pour docs/CHANTIERS_DETAIL.md (NON committé, à recopier)

### F1 — modèle de données de la refonte

- **Problème** : schéma du calculateur/monitor 30 j inadapté au graphe CM1/CM2/CM3, aux
  faits temps réel et aux sources externes ; liste de purge RGPD dupliquée dans deux webhooks.
- **Solution** : 23 migrations (26 tables, 2 fonctions RGPD), `order_margins` étendue en place
  (colonnes nullables, aucun snapshot réécrit, rename différé à F2), `shop_settings` scindée de
  `shop_plans`, `sync_jobs`/`alert_state` avec copie unique des états existants, purge totale
  par `purge_shop` + repli JS piloté par `app/lib/schema.js`, chiffrement AES-256-GCM des
  jetons, `lot23_schema` (95 assertions : RLS partout, ré-exécutabilité, contrat de liste,
  purge = liste, immuabilité des snapshots, chiffrement).
- **Reporté** : rename `order_lines` (F2), suppression de `calculations`/`margin_alerts` (avec
  le code, F4).

---

## Annexe A — sortie integrale de npm test (23 lots)
...
── 1. RLS deny-all sur chaque table créée par F1 ──
  ✓ 23 migrations F1 présentes (trouvées : 23)
  ✓ shop_settings : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_01_shop_settings.sql)
  ✓ orders : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_02_orders.sql)
  ✓ refunds : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_04_refunds.sql)
  ✓ returns : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_05_returns.sql)
  ✓ fulfillments : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_06_fulfillments.sql)
  ✓ order_fees : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_07_order_fees.sql)
  ✓ inventory_daily : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_08_inventory_daily.sql)
  ✓ sessions_daily : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_09_sessions_daily.sql)
  ✓ ad_spend : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_10_ad_spend.sql)
  ✓ ad_entities : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_10_ad_spend.sql)
  ✓ seo_daily : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_11_seo_daily.sql)
  ✓ seo_index_status : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_11_seo_daily.sql)
  ✓ fx_rates : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_12_fx_rates.sql)
  ✓ partners : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_14_partners_promo_codes.sql)
  ✓ promo_code_rules : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_14_partners_promo_codes.sql)
  ✓ manual_commissions : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_14_partners_promo_codes.sql)
  ✓ fixed_costs : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_15_fixed_costs.sql)
  ✓ integration_connections : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_16_integration_connections.sql)
  ✓ sync_jobs : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_17_sync_jobs.sql)
  ✓ webhook_events : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_18_webhook_events.sql)
  ✓ daily_rollups : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_19_daily_rollups.sql)
  ✓ product_daily_rollups : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_19_daily_rollups.sql)
  ✓ customers_agg : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_20_customers_agg.sql)
  ✓ alert_rules : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_21_alerts.sql)
  ✓ alert_state : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_21_alerts.sql)
  ✓ ai_explanations : RLS activée + DROP POLICY IF EXISTS + politique deny_public_access (20260922_f1_22_ai_explanations.sql)
  ✓ 26 tables créées par F1 (trouvées : 26)

── 2. Migrations F1 ré-exécutables ──
  ✓ 20260922_f1_01_shop_settings.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_02_orders.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_03_order_margins_extend.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_04_refunds.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_05_returns.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_06_fulfillments.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_07_order_fees.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_08_inventory_daily.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_09_sessions_daily.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_10_ad_spend.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_11_seo_daily.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_12_fx_rates.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_13_variant_costs_extend.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_14_partners_promo_codes.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_15_fixed_costs.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_16_integration_connections.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_17_sync_jobs.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_18_webhook_events.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_19_daily_rollups.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_20_customers_agg.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_21_alerts.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_22_ai_explanations.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout
  ✓ 20260922_f1_23_rgpd_functions.sql : IF NOT EXISTS / OR REPLACE / DROP POLICY IF EXISTS partout

── 3. schema.js = ensemble des tables créées par les migrations ──
  ✓ aucune table SQL absente de schema.js
  ✓ aucune table de schema.js absente des migrations
  ✓ 12 tables historiques + 26 tables F1 (12 + 26)
  ✓ aucun doublon dans ALL_TABLES
  ✓ F1_TABLES = exactement les tables des fichiers 20260922_f1_*

── 4. purge_shop = PURGE_TABLES ; webhooks sans liste en dur ──
  ✓ chaque table de PURGE_TABLES est vidée par purge_shop
  ✓ purge_shop ne vide rien hors PURGE_TABLES
  ✓ aucune table vidée deux fois
  ✓ fx_rates (référentiel partagé) hors purge
  ✓ enfants (commissions, codes) vidés avant partners (FK)
  ✓ annotations vidées avant calculations (FK)
  ✓ purge totale : toutes les tables sauf les référentiels partagés (décision g)
  ✓ webhooks.app.uninstalled.jsx : appelle la RPC purge_shop
  ✓ webhooks.app.uninstalled.jsx : aucun nom de table en dur (repli piloté par PURGE_TABLES)
  ✓ webhooks.app.uninstalled.jsx : repli sur la liste partagée schema.js
  ✓ webhooks.compliance.jsx : appelle la RPC purge_shop
  ✓ webhooks.compliance.jsx : aucun nom de table en dur (repli piloté par PURGE_TABLES)
  ✓ webhooks.compliance.jsx : repli sur la liste partagée schema.js
  ✓ customers/redact : RPC redact_customer avec le gid brut (décision b)

── 5. order_margins : contrat d'immuabilité ──
  ✓ clé d'idempotence (shop_domain, order_id, line_item_id) présente dans la migration d'origine
  ✓ 8 colonnes ajoutées à order_margins (trouvées : 8)
  ✓ aucune colonne ajoutée n'a de DEFAULT ni de NOT NULL (aucun snapshot réécrit)
  ✓ F1 ne renomme ni ne supprime rien sur order_margins (rename en F2 avec le code)
  ✓ les nouvelles composantes CM1/CM2 sont déclarées colonnes de snapshot dans schema.js
  ✓ 20260922_f1_01_shop_settings.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_02_orders.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_03_order_margins_extend.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_04_refunds.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_05_returns.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_06_fulfillments.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_07_order_fees.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_08_inventory_daily.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_09_sessions_daily.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_10_ad_spend.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_11_seo_daily.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_12_fx_rates.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_13_variant_costs_extend.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_14_partners_promo_codes.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_15_fixed_costs.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_16_integration_connections.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_17_sync_jobs.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_18_webhook_events.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_19_daily_rollups.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_20_customers_agg.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_21_alerts.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_22_ai_explanations.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ 20260922_f1_23_rgpd_functions.sql : aucun UPDATE d'une colonne de snapshot d'order_margins
  ✓ orders.excluded_reason : le CHECK SQL porte exactement les raisons de schema.js

── 6. Chiffrement AES-256-GCM des jetons ──
  ✓ aller-retour : déchiffré === original (UTF-8 conservé)
  ✓ deux chiffrements du même jeton diffèrent (IV aléatoire)
  ✓ le texte chiffré ne contient pas le jeton en clair
  ✓ clé absente → erreur explicite (jamais de stockage en clair)
  ✓ clé de mauvaise longueur → erreur
  ✓ mauvaise clé → échec d'authentification GCM (pas de texte corrompu rendu)

── 7. redact_customer ──
  ✓ supprime la ligne customers_agg du client
  ✓ anonymise les commandes du client (identifiant et rang à NULL)
  ✓ ne supprime AUCUNE commande (les montants restent, non nominatifs)
...
## Annexe B — git diff des fichiers suivis (les nouveaux fichiers sont listes en section Perimetre)

```diff
diff --git a/app/routes/webhooks.app.uninstalled.jsx b/app/routes/webhooks.app.uninstalled.jsx
index c535082..7f93d9d 100644
--- a/app/routes/webhooks.app.uninstalled.jsx
+++ b/app/routes/webhooks.app.uninstalled.jsx
@@ -1,6 +1,7 @@
...
diff --git a/app/routes/webhooks.compliance.jsx b/app/routes/webhooks.compliance.jsx
index 457ad39..134759c 100644
--- a/app/routes/webhooks.compliance.jsx
+++ b/app/routes/webhooks.compliance.jsx
@@ -1,6 +1,7 @@
 import { authenticate } from "../shopify.server";
 import { supabase } from "../supabase.server";
 import db from "../db.server";
...
@@ -11,37 +12,37 @@ export const action = async ({ request }) => {
