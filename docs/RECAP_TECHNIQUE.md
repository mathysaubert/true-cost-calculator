...
**Coûts par variante — Brique A** — `variantCosts.js`, table `variant_costs`, actions `costs_list` (LECTURE SEULE depuis l'ère XV), `costs_save`, `costs_import_csv` (`costs_confirm_all` **neutralisée**). `costs_list` construit les lignes affichées via `buildCostRowsForDisplay` (pur, aucune écriture) : ligne stockée renvoyée telle quelle (`stored:true`), sinon suggestion estimée (`estimateVariantCost`, taguée `stored:false`, jamais persistée). Valide (`validateCostRow`, refuse `prix_achat ≤ 0`), importe/exporte CSV (`parseCostsCsv`/`buildCostsCsv`). Trois `cost_source` : `estimated` (deviné, affichage seul), `confirmed` (saisi), `imported` (CSV) ; `confirmed`/`imported` **immuables** (voir §5). Statut produit : `productCostStatus` (Coûts complets / À compléter / Partiel).
...
### Autorité de la donnée marchand — `confirmed`/`imported` immuables (ère XV)

Depuis l'ère XV, `costs_list` est en **lecture seule** (« plus jamais de pré-rempli ») : plus aucune réhydratation-écriture. L'ancien helper `reconcileEstimatedCost` (qui corrigeait le `prix_achat` d'une ligne `estimated` depuis le `unitCost` Shopify) a été **supprimé** avec ses tests — code mort de réécriture, piège d'intégrité. La garantie **« la donnée marchand fait autorité »** demeure, portée autrement : `buildCostRowsForDisplay` renvoie une ligne stockée (`confirmed`/`imported`) telle quelle, et une variante sans ligne reçoit une suggestion `stored:false` (affichage, jamais écrite) ; `validateCostRow` refuse tout `prix_achat ≤ 0`. On ne détruit/écrase jamais une donnée que le marchand a validée — même invariant que le recalcul.
...
| Fiabilité des marges réelles (ère XV) | **coûts confirmés ou importés** (jamais « confirmés » seul) · **ventes analysées** (dénominateur = confirmed+imported+estimated) | « marge inconnue » (vente `missing`, sans coût renseigné) | compteur de fiabilité (Suivi) |
| Complétude des coûts d'un produit (ère XV) | **Coûts complets** / **À compléter** / **Partiel : N variantes sur M** | badge de statut de la liste | liste produits (Suivi) |
| Valeur estimée non saisie (ère XV) | **suggestion** (affichée en placeholder « ex : … ») | — | panneau d'édition (Suivi) |
...
### Architecture des onglets Audit & Suivi (ère XV — « valeur d'abord »)

Principe : chaque onglet ouvre sur la VALEUR (le chiffre déjà calculé), pas sur du travail. Réorganisation d'interface **autour** des actions éprouvées — zéro action nouvelle, zéro action réécrite (`costs_confirm_all` neutralisée = seule exception, sens *retrait*).

- **Suivi des coûts** — ordre : intro → [1] résumé réel compact (`CostSummaryBanner`, toujours visible) → [2] compteur de fiabilité (`ReliabilityCounter`) → [3] Marge réelle (sync/compléter/corriger) → [3bis] `MarginMonitor` complet (accordéon inchangé) → [4] classification douanière → [5] import/export CSV → [6] cartes cadeaux → [7] liste produits (`ProductCostList`) + panneau d'édition (`ProductCostPanel`) → [8] réglages. Composants extraits **rendables** dans `app/components/costsUi.jsx` (harness `render_check.mjs`).
- **`costs_list` en LECTURE SEULE** : `buildCostRowsForDisplay` (pur, `variantCosts.js`) — aucun chemin d'écriture. Variante sans ligne stockée → suggestion `stored:false` (placeholder), jamais persistée. Seuls `costs_save` / `costs_import_csv` écrivent. Un compte neuf qui ouvre le Suivi crée **zéro** ligne.
- **Compteur de fiabilité** : `computeCostReliability` (pur, `orderHistory.js`) — somme de champs figés (`cost_source`, `line_net_revenue`, `effective_qty`), **aucun recalcul moteur**. Option A : X % sur les ventes *analysées*, `missing` à part.
- **Intégrité de saisie** : suggestions en placeholder (jamais donnée) ; enregistrement par produit, variantes touchées seulement ; `validateCostRow` **refuse `prix_achat ≤ 0`** (message « Indiquez le prix d'achat fournisseur ») → jamais de coût fictif « confirmé ».
- **Audit Catalogue** — auto-lancement Expert (résultat hissé dans l'état parent, survit onglets + quota), ligne « Hypothèses · Affiner » (paramètres repliés, `run_audit` inchangée), guide 2 lignes + `<details>`, état vide guidé. Réserve : conteneur non couvert par le harness isolé (validé build SSR + eslint + smoke live).
...
- **Dette — `costs_confirm_all` neutralisée sans test exécutable** (ère XV) : l'action « Tout confirmer » a été retirée de l'UI et l'action serveur rendue inerte (erreur explicite, zéro écriture). Sa neutralisation est vérifiée **par revue + grep** (« Tout confirmer » = 0 en UI), **pas par un test exécutable** (l'action est couplée à `supabase`/session, non testable en lot pur sans mock). **Ne pas réactiver sans chantier dédié** — confirmer en masse des coûts estimés rouvrirait le pré-rempli fictif.
- **Backlog — rappel périodique de vérification des coûts** (ère XV) : cron mensuel opt-in rappelant au marchand de vérifier ses coûts fournisseur (prix, port, emballage) pour que marges et alertes restent justes. Non implémenté — post-bêta, sur demande utilisateur. Le contrat de déclenchement (email d'alerte + onglet Alertes) pose déjà l'attente côté marchand.
- **Backlog — CA brut des lignes `missing`** (ère XV) : le compteur de fiabilité (`computeCostReliability`) classe les produits « à compléter » par **unités vendues** (`effective_qty`), seul signal disponible : une ligne `missing` ne stocke aucun CA (`line_net_revenue: null`, `orderIngest.js`). Pour classer par CA (« vos plus gros vendeurs »), il faudrait **stocker le CA brut à l'ingestion** (mutation de schéma + re-sync) — reporté post-bêta, sur demande utilisateur. Option A retenue : « X % de vos ventes analysées » (dénominateur = confirmed+imported+estimated), `missing` comptées à part.
- **`reconcileEstimatedCost` supprimé** (ère XV, pour mémoire) : ancien helper de réhydratation `prix_achat` ← `unitCost` Shopify. Sans objet depuis que `costs_list` est en lecture seule (« plus jamais de pré-rempli ») ; retiré avec ses tests. La garantie « donnée marchand fait autorité » est portée par `buildCostRowsForDisplay` (ligne stockée renvoyée telle quelle) + le refus de tout `prix_achat ≤ 0` dans `validateCostRow`.
