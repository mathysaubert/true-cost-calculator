...
### 67. Incident prod (22/07) — crash « i is not iterable » de l'onglet Suivi des coûts — `510d49b`
- **Problème** : après le déploiement de `fdc53e1`, ouvrir l'onglet **Suivi des coûts** en prod déclenchait l'error boundary « L'application n'a pas pu se charger. i is not iterable ». Régression introduite par le chantier douane ; prod rollback sur `27c5a6b` le temps du correctif.
