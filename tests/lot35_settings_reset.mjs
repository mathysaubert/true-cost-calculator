...
//  app/components/settings/Fields.jsx, chargés par Vite. Les éléments s-text-field / s-select
//  reproduisent le contrat lu dans polaris.js (cdn.shopify.com/shopifycloud/polaris.js) :
//    • attribut `value` ⇔ propriété `defaultValue` (valeur de départ) ;
//    • propriété `value` = valeur saisie, repli sur defaultValue tant que rien n'est saisi ;
//    • formResetCallback() { this.value = this.defaultValue || "" } (appelé par form.reset()).
//  jsdom n'appelle pas formResetCallback (pas d'ElementInternals) : resetForm() fait ce que fait le
//  navigateur pour un élément associé au formulaire. Chemins prouvés : arrivée par le menu (rendu
//  client), page chargée en entier (rendu serveur + hydratation), valeur ré-enregistrée, champ vide.
...
class PolarisFieldContract extends dom.window.HTMLElement {
...
dom.window.customElements.define("s-text-field", class extends PolarisFieldContract {});
dom.window.customElements.define("s-select", class extends PolarisFieldContract {});
dom.window.customElements.define("s-option", class extends dom.window.HTMLElement {});
...
console.log("\n── 1. Arrivée par le menu (rendu client React 19) ──");
{
  ...
  ok(f.num.getAttribute("value") === "0.8" && f.txt.getAttribute("value") === "Abonnement" && f.date.getAttribute("value") === "2026-09-01" && f.sel.getAttribute("value") === "franchise", "valeur de départ posée (attribut value) sur les 4 types de champ");
  typeAll(f);
