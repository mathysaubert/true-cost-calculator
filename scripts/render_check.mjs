...
check("port, règle « non renseigné » : défaut vide + surcharge FR → « À confirmer » ; rien → « Manquant » (rouge), défaut vide ; 0 € confirmé → « Renseigné », value=\"0\"",
  wrap("fr", React.createElement("div", null,
    React.createElement(ShippingForm, { settings: { shipping_cost_rules: { default: null, byCountry: { FR: 3 }, confirmed: true } } }),
    React.createElement(ShippingForm, { settings: { shipping_cost_rules: { default: null, byCountry: {}, confirmed: false } } }),
    React.createElement(ShippingForm, { settings: { shipping_cost_rules: { default: 0, byCountry: {}, confirmed: true } } }))),
  (h) => { const parts = h.split('aria-labelledby="tcc-shipping-title"').slice(1); return parts.length === 3 && /data-shipping-state="unconfirmed">À confirmer</.test(parts[0]) && /name="shipping_default"[^>]*value=""/.test(parts[0]) && /name="shipping_country_1"[^>]*value="FR"/.test(parts[0]) && /tcc-badge--bad" data-shipping-state="unset">Manquant</.test(parts[1]) && /name="shipping_default"[^>]*value=""/.test(parts[1]) && /tcc-badge--good" data-shipping-state="set">Renseigné</.test(parts[2]) && /name="shipping_default"[^>]*value="0"/.test(parts[2]); });
...
check("Marketing : 2 partenaires (Supprimer), formulaire d'ajout avec mode ; règles : « Codes vus sans règle : WELCOME (3 commandes) », 1 règle TEST20 12,5 % HT après remise, formulaire (partenaire, base, dates) ; commissions manuelles : 1 ligne, formulaire limité au partenaire manuel",
  ...
  (h) => (h.match(/data-partner="/g) ?? []).length === 2 && /name="intent" value="add_partner"/.test(h) && /<s-select name="mode"/.test(h) && /Codes vus dans vos commandes sans règle : WELCOME \(3 commandes\)\./.test(h) && /data-code="TEST20"[\s\S]*?Agence A[\s\S]*?12,5.%[\s\S]*?HT, après remise/.test(h) && /<s-select name="partner_id"[^>]*value=""/.test(h) && /<s-option value="p1"(?: selected="")?>Agence A</.test(h) && /data-commission="c1"[\s\S]*?Influ B[\s\S]*?2026-09 · post/.test(h) && /name="intent" value="add_manual_commission"[\s\S]*?<s-option value="p2"(?: selected="")?>Influ B</.test(h) && !/<s-option value="p1"(?: selected="")?>Agence A<\/s-option>[\s\S]*?name="period_month"/.test(h.split('value="add_manual_commission"')[1] ?? ""));
...
check("douane (en) : 1 produit à confirmer (Tee, catégorie stockée Textile présélectionnée), formulaire intent confirm_customs ; Poster (aucune variante stockée) absent",
  ...
  (h) => /Customs classification: 1 product to confirm/.test(h) && /data-customs="gid:\/\/shopify\/Product\/A"/.test(h) && !/data-customs="gid:\/\/shopify\/Product\/B"/.test(h) && /<s-select name="categorie"[^>]*value="Textile"/.test(h) && /<s-option value="Électronique"(?: selected="")?>Electronics</.test(h) && /value="confirm_customs"/.test(h));
check("CSV (fr) : export, import multipart ; 8 importées, 16 « à compléter » à part (pas des erreurs), 2 rejets avec raison, valeur reçue et valeurs acceptées",
  wrap("fr", React.createElement(CostsCsv, { result: { intent: "import_csv", ok: true, saved: 8, incomplete: 16, errorCount: 2, csvErrors: [{ line: 4, issues: [{ field: "prix_achat", reason: "not_number", value: "abc" }, { field: "categorie", reason: "unknown_value", value: "Electro", expected: ["Textile", "Électronique"] }] }, { line: 7, issues: [{ field: "variant_id", reason: "missing_id", value: "" }] }] } })),
  (h) => /<s-button variant="primary" data-export="xlsx">Exporter le modèle Excel \(\.xlsx\)<\/s-button>/.test(h) && /<s-button variant="tertiary" data-export="csv">CSV \(UTF-8\)<\/s-button>/.test(h) && !/data:text\/csv/.test(h) && /Importer ou exporter \(Excel ou CSV\)/.test(h) && /Fichier \(\.xlsx ou \.csv\)/.test(h) && /accept="\.xlsx,\.csv/.test(h) && /enctype="multipart\/form-data"/i.test(h) && /<input type="file"[^>]*name="csv"/.test(h) && /8 variantes importées\. 2 lignes ont été rejetées\./.test(h) && /data-csv-incomplete="16"[^>]*>16 lignes restent à compléter : un coût est vide \(non renseigné\)/.test(h) && /Ligne 4 : prix d(?:&#x27;|')achat « abc » : ce n(?:&#x27;|')est pas un nombre ; catégorie « Electro » : valeur inconnue \(acceptées : Textile, Électronique\)/.test(h) && /Ligne 7 : identifiant de variante : manquant/.test(h));
check("CSV (fr) : ancien .xls, fichier trop gros, fichier illisible → messages clairs",
  wrap("fr", React.createElement("div", null, ...["unsupported_format", "too_large", "unreadable"].map((reason) => React.createElement(CostsCsv, { key: reason, result: { intent: "import_csv", ok: false, error: "header", saved: 0, header: { reason } } })))),
  (h) => /data-csv-header="unsupported_format">C(?:&#x27;|')est un ancien fichier Excel \(\.xls\)/.test(h) && /data-csv-header="too_large">Le fichier dépasse 5 Mo/.test(h) && /data-csv-header="unreadable">Le fichier n(?:&#x27;|')a pas pu être lu comme un classeur Excel/.test(h) && !/tone="critical">Échec/.test(h));
check("CSV (en) : en-tête incomplet → message clair, rien d'importé",
  wrap("en", React.createElement(CostsCsv, { result: { intent: "import_csv", ok: false, error: "header", saved: 0, header: { reason: "missing_columns", columns: ["categorie"] } } })),
  (h) => /data-csv-header="missing_columns">Columns missing from the header: categorie\. Start again from the exported template\./.test(h) && !/variants? imported/.test(h));
