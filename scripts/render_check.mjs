...
check("CSV (fr) : export, import multipart ; 8 importées, 16 « à compléter » à part (pas des erreurs), 2 rejets avec raison, valeur reçue et valeurs acceptées",
  wrap("fr", React.createElement(CostsCsv, { csv: "variant_id,prix_achat\ngid,1", result: { intent: "import_csv", ok: true, saved: 8, incomplete: 16, errorCount: 2, csvErrors: [{ line: 4, issues: [{ field: "prix_achat", reason: "not_number", value: "abc" }, { field: "categorie", reason: "unknown_value", value: "Electro", expected: ["Textile", "Électronique"] }] }, { line: 7, issues: [{ field: "variant_id", reason: "missing_id", value: "" }] }] } })),
  (h) => /href="data:text\/csv;charset=utf-8,variant_id%2Cprix_achat/.test(h) && /download="true-cost-calculator-costs\.csv"/.test(h) && /enctype="multipart\/form-data"/i.test(h) && /<input type="file"[^>]*name="csv"/.test(h) && /8 variantes importées\. 2 lignes ont été rejetées\./.test(h) && /data-csv-incomplete="16"[^>]*>16 lignes restent à compléter : un coût est vide \(non renseigné\)/.test(h) && /Ligne 4 : prix d(?:&#x27;|')achat « abc » : ce n(?:&#x27;|')est pas un nombre ; catégorie « Electro » : valeur inconnue \(acceptées : Textile, Électronique\)/.test(h) && /Ligne 7 : identifiant de variante : manquant/.test(h));
check("CSV (en) : en-tête incomplet → message clair, rien d'importé",
  wrap("en", React.createElement(CostsCsv, { csv: "", result: { intent: "import_csv", ok: false, error: "header", saved: 0, header: { reason: "missing_columns", columns: ["categorie"] } } })),
